// 简易 QR 码生成器（纯 JS，无外部依赖）
// 基于 Kazuhiko Arase 的 qrcode-generator (MIT License)
// 精简版，仅支持 SVG 输出
...
var QRCodeLib = (function() {
  ...
  var QRUtil = {
    ...
    getLostPoint: function(qrCode) {
      ...
      for (var col = 0; col < moduleCount; col++) {
        for (var row = 0; row < moduleCount; row++) {
          if (qrCode.modules[row][col]) darkCount++;
        }
      }
  ...
  QRRSBlock.RS_BLOCK_TABLE = [
    ...
    [3,145,115,1,146,116],[4,64,40,5,65,41],[11,54,24,5,55,25],[11,54,16,5,55,17],
    [5,109,87,1,110,88],[5,65,41,5,66,42],[5,54,24,7,55,25],[11,54,16,7,55,17],
  ...
  // ===== SVG 输出 =====
  function toSVG(qrCode, size, margin) {
    margin = margin || 4;
    size = size || 200;
    var moduleCount = qrCode.moduleCount;
    var cellSize = size / moduleCount;
    var totalSize = size + margin * 2 * cellSize;
    var svg = '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ' + totalSize + ' ' + totalSize + '" width="' + size + '" height="' + size + '">';
    svg += '<rect width="100%" height="100%" fill="white"/>';
    for (var row = 0; row < moduleCount; row++) {
      for (var col = 0; col < moduleCount; col++) {
        if (qrCode.modules[row][col]) {
          var x = col * cellSize + margin * cellSize;
          var y = row * cellSize + margin * cellSize;
          svg += '<rect x="' + x.toFixed(2) + '" y="' + y.toFixed(2) + '" width="' + cellSize.toFixed(2) + '" height="' + cellSize.toFixed(2) + '" fill="black"/>';
        }
      }
    }
    svg += '</svg>';
  ...
  // 自动选择最小版本号
  function getTypeNumber(data, errorCorrectLevel) {
  ...
  return {
    // 生成 SVG 字符串
    toSVGString: function(data, size, margin) {
      size = size || 200;
      margin = margin || 2;
      var typeNumber = getTypeNumber(data, QRErrorCorrectLevel.M);
      var qr = new QRCode(typeNumber, QRErrorCorrectLevel.M);
      qr.addData(data);
      qr.make();
      return toSVG(qr, size, margin);
    },
    
    // 生成 Data URL
    toDataURL: function(data, size, margin) {
      var svg = this.toSVGString(data, size, margin);
